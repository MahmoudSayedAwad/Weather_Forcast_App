...
class AlertsAdapter(
    var context: Context?, var language: String,
    var onDeleteAlert: OnDeleteAlert
) : ListAdapter<UserAlertsEntity, AlertsViewHolder>(
    AlertsDiffUtile()
) {
    ...
    override fun onBindViewHolder(holder: AlertsViewHolder, position: Int) {
        ...
        holder.binding.startAlertDateTextViewRow.text =
            getDateTimeLong(current.startDate, "EEE, MMM d", language)
        holder.binding.startAlertTimeTextViewRow.text =
            getDateTimeLong(current.startDate, "hh:mm a", language)

        holder.binding.endAlertTimeTextViewRow.text =
            getDateTimeLong(current.endDate, "hh:mm a", language)
        holder.binding.endAlertDateTextViewRow.text =
            getDateTimeLong(current.endDate, "EEE, MMM d", language)
